...
class TommySugoCalendar:
    ...
    def get_current_event(marketing_window_days: int = 14) -> Optional[Dict]:
        ...
        events = [
            ...
            {
                "name": "Easter",
                "date": datetime(current_year, 4, 5),
                "market_from_days_before": 26,
                "messages": [
            ...
            },
            {
                ...
                "date": datetime(current_year, 4, 11),
                "market_from_days_before": 22,
                "messages": [
            ...
            },
            {
                ...
                "date": datetime(current_year, 7, 1),
                "market_from_days_before": 60,
                "messages": [
            ...
            {
                "name": "Father's Day",
                "date": datetime(current_year, 9, 6),
                "market_from_days_before": 22,
                "messages": [
                    "Dad would love this!",
            ...
            },
            {
                ...
                "date": datetime(current_year, 9, 15),
                "market_from_days_before": 30,
                "messages": [
            ...
            },
            {
                ...
                "date": datetime(current_year, 12, 24),
                "market_from_days_before": 14,
                "messages": [
        ...
        for event in events:
            ...
            market_end = event_date + timedelta(days=1)
...
{json.dumps(fields_to_validate, indent=2)}"""
    ...
    def generate_feedback_response(self, customer_feedback: str, customer_name: str) -> str:
        """
        Generate a warm, personalized response that REFERENCES the actual feedback.
        The response feels personal - like Nathan himself is replying.
        """
        ...
        try:
            ...
            # ✅ FIX: Better prompt that references actual feedback + feels personal from Nathan
            prompt = f"""Customer "{first_name}" left this feedback about Tommy Sugo:
"{customer_feedback}"

Write a short, warm and natural reply that:

Clearly responds to what they mentioned (refer to the specific dish, experience, or issue — but don’t repeat their exact words)

Sounds human, fuzzy, and heartfelt — not corporate or scripted

Is 1–2 short sentences (maximum 40 words)

Has a sweet, light tone with a tiny touch of humour where appropriate

If the feedback is negative, respond calmly, show genuine care, and reassure them we’ll make it right

Do NOT mention Nathan, the owner, or “the Tommy Sugo team”

Do NOT start with “Dear”

Do NOT use stiff phrases like “we are very glad” or “we apologise for the inconvenience”

No sign-off needed

The reply should feel like it came from a real person who truly cares about pasta and people.

Now write the response for {first_name}'s feedback above:"""
            
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are Nathan, the warm and passionate owner of Tommy Sugo, an Italian food business in Perth. Write genuine, heartfelt replies that reference the customer's actual feedback. Never be generic. Be like a small business owner who truly cares."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.8,
                max_tokens=150
            ...
            feedback_response = feedback_response.strip('"').strip("'")
...
Write ONE short, friendly marketing line (max 30-50 words) recommending ONE unordered product.{' Make it relevant to the event if mentioned.' if event_context else ''}"""
            ...
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "Write SHORT, friendly, casual marketing recommendations. Connect products to events/holidays when relevant. Engage customers warmly. Make it at least 30-50 words (max)."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.7,
                max_tokens=1000
